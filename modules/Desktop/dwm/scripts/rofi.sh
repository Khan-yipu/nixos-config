...
##### MAIN_MENU ####
    ...
    main_menu_cmds=(
        'feh --randomize --bg-fill ~/Pictures/wallpapers/*.png; show_main_menu' # 执行完不退出脚本继续执行show_main_menu
...
##### TOGGLE_SERVER_MENU #####
    ...
    # toggle_server_menu_cmds[2]='coproc (picom --experimental-backends --config $DWM/scripts/config/picom.conf > /dev/null 2>&1)'
    toggle_server_menu_cmds[2]='coproc (picom --config $DWM/scripts/config/picom.conf > /dev/null 2>&1)'
